...
CGame::CGame(string brd, string clr, string cstl, string ep, string hm,
             string fm)
  ...
  *m |= enPassantFile << 18;
  ...
  switch (special) {
    ...
    case CASTLE: {
      ...
      movePiece(wtm, ROOK, rookSource, rookDest, exp_2(rookSource),
                exp_2(rookDest));
    }
    case KING_MOVE: {
...
void CGame::unmakeMove(mv m) {
  ...
  switch (special) {
    ...
    case CASTLE: {
      ind rookSource = dest > source ? dest + 2 : dest - 1;
      ind rookDest = dest > source ? dest - 1 : dest + 1;
      movePiece(wtm, ROOK, rookDest, rookSource, exp_2(rookDest),
                exp_2(rookSource));
      break;
    }
    case PROMOTE_QUEEN:
