...
// CBoard contains the state of the board. It's main method is moveGen, so, in
// principal, it should  have just enough information to generate the legal
...
class CBoard {
 public:
  ...
  // The interface
  void setBoard(string brd = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR",
                string clr = "w", string cstl = "KQkq", string ep = "-");
  void moveGen(mv *moveList);
  void capGen(mv *moveList);
  void moveGen(mv *moveList, bool quietMoves);
  void movePiece(bool color, ind attacker, ind source, ind dest, BB sourceBB,
                 BB destBB);
  ...
  CBoard(string brd = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR",
         string clr = "w", string cstl = "KQkq", string ep = "-");
  ~CBoard();
 ...
 protected:
  ...
  void generateMovesTo(mv **moveList, ind square, ind defender, BB pins,
                       BB enemyAttacks);
  ...
  void serializeFromDest(mv **moveList, BB b, ind dest, ind defender,
                         ind special);
