...
void CBoard::moveGen(mv *moveList) { moveGen(moveList, true); }

// Generate legal captures or check evasions given the current state of the
// board.
void CBoard::capGen(mv *moveList) { moveGen(moveList, false); }
...
bool CBoard::inCheck() { return (pieces[wtm][KING] & attackSetGen(!wtm)) != 0; }
...
void CBoard::pawnGen(mv **moveList, BB pins, bool quietMoves) {
  ...
  if (quietMoves) {
    ...
    if (wtm) {
      ...
      serializePawn(moveList, ((b & masks::RANK[2]) << 8) & ~occupied,
                    DOUBLE_PAWN_MOVE_W, 16);
    } else {
      BB b = (pawns >> 8) & ~occupied;
      serializePawn(moveList, b, REGULAR_MOVE, -8);
      serializePawn(moveList, ((b & masks::RANK[5]) >> 8) & ~occupied,
...
void CBoard::pawnCaps(mv **moveList, BB pins) {
  ...
  if (enPassant) {
    do {
      ...
      BB candidateSquares = wtm ? (enPassant >> 7 | enPassant >> 9)
                                : (enPassant << 7 | enPassant << 9);
      ...
      serializeFromDest(moveList, candidateSquares, enPassantSquare, PAWN,
                        EN_PASSANT_CAP);
...
void CBoard::pawnGenPinned(mv **moveList, BB pins, ind kingSquare,
                           bool quietMoves) {
  BB pawns = pieces[wtm][PAWN] & pins;
  while (pawns) {
    ...
    if (wtm) {
      ...
      if (quietMoves && (squares::file(square) == squares::file(kingSquare))) {
        ...
        if (moves & masks::RANK[2]) {
          serializePawn(moveList, (moves << 8) & ~occupied, DOUBLE_PAWN_MOVE_W,
                        16);
        }
      }
    } else {
      ...
      if (quietMoves && (squares::file(square) == squares::file(kingSquare))) {
        ...
        if (moves & masks::RANK[5]) {
          serializePawn(moveList, (moves >> 8) & ~occupied, DOUBLE_PAWN_MOVE_B,
                        -16);
        }
      }
    }
    ...
    // Captures
    BB pawnThreats = ((pawnShift << 1) | (pawnShift >> 1)) &
                     masks::OPPOSITE[kingSquare][square];
    ind targetSquare = bitscan(pawnThreats);
    if (pawnThreats & pieces[!wtm][ALL]) {
      serializeFromDest(moveList, exp_2(square), targetSquare,
                        board[targetSquare], REGULAR_MOVE);
    } else if (pawnThreats & enPassant) {
      serializeFromDest(moveList, exp_2(square), targetSquare, PAWN,
                        EN_PASSANT_CAP);
    }
...
void CBoard::knightGen(mv **moveList, BB pins, bool quietMoves) {
  ...
  while (knights) {
    ...
    BB movesBB = masks::KNIGHT_MOVES[i] &
                 (quietMoves ? ~pieces[wtm][ALL] : pieces[!wtm][ALL]);
...
void CBoard::bishopGenPinned(mv **moveList, BB pins, ind kingSquare,
                             bool quietMoves) {
  BB sliders = (pieces[wtm][BISHOP] | pieces[wtm][QUEEN]) & pins;
  while (sliders) {
    ind i = bitscan(sliders);
    BB validMoves =
        masks::OPPOSITE[kingSquare][i] | masks::INTERCEDING[kingSquare][i];
...
// whether to return non-captures.
void CBoard::rookGenPinned(mv **moveList, BB pins, ind kingSquare,
                           bool quietMoves) {
  BB sliders = (pieces[wtm][ROOK] | pieces[wtm][QUEEN]) & pins;
  while (sliders) {
    ind i = bitscan(sliders);
...
// Optionally, only generate captures by passing quietMoves=false.
void CBoard::kingGen(mv **moveList, ind kingSquare, BB enemyAttacks,
                     bool quietMoves) {
  // Regular moves
  BB validSquares =
      ~enemyAttacks & (quietMoves ? ~pieces[wtm][ALL] : pieces[!wtm][ALL]);
...
void CBoard::generateMovesTo(mv **moveList, ind dest, ind defender, BB pins,
                             BB enemyAttacks) {
  // Pawn moves
  ...
  if (defender) {
    ...
    serializeFromDest(moveList,
                      pawns & ((potentialPawns & ~masks::FILE[0]) << 1), dest,
                      PAWN, EN_PASSANT_CAP);
    serializeFromDest(moveList,
                      pawns & ((potentialPawns & ~masks::FILE[7]) >> 1), dest,
                      PAWN, EN_PASSANT_CAP);
  } else {
    if (pawns & potentialPawns) {
      ...
      if (wtm && squares::rank(dest) == 3) {
        serializeFromDest(moveList, pawns & (potentialPawns >> 8), dest, NONE,
                          DOUBLE_PAWN_MOVE_W);
...
void CBoard::evasionGen(mv **moveList, BB enemyAttacks, BB pins,
                        ind kingSquare) {
  ...
  attackers[BISHOP] = magics::bishopMoves(kingSquare, occupied) &
                      (pieces[!wtm][BISHOP] | pieces[!wtm][QUEEN]);
  attackers[ROOK] = magics::rookMoves(kingSquare, occupied) &
  ...
  if (popcount(attackers[ALL]) == 1) {
    ...
    if (attackers[BISHOP] | attackers[ROOK]) {
      ...
      BB candidatePawns = masks::KING_MOVES[enPassantSquare] &
                          masks::KING_MOVES[attackerSquare];
      candidatePawns &= pieces[wtm][PAWN] & ~pins;
      if (candidatePawns) {
        serializeFromDest(moveList, candidatePawns, enPassantSquare, PAWN,
                          EN_PASSANT_CAP);
      }
    }
  ...
  BB kingMoves =
      masks::KING_MOVES[kingSquare] & ~pieces[wtm][ALL] & ~enemyAttacks;
  for (BB x = kingMoves; x; x &= x - 1) {
    ind i = bitscan(x);
    if (masks::OPPOSITE[i][kingSquare] &
...
BB CBoard::bishopPins(ind kingSquare) {
  BB pins = 0;
  BB candidatePins =
      magics::bishopMoves(kingSquare, occupied) & pieces[wtm][ALL];
  while (candidatePins) {
    ...
    if (candidateMoves & kingXRayAttacks &
        (pieces[!wtm][BISHOP] | pieces[!wtm][QUEEN])) {
      pins |= exp_2(i);
    }
...
void CBoard::serialize(mv **moveList, BB b, ind source, ind special) {
  while (b) {
    ind dest = bitscan(b);
    *((*moveList)++) = moves::make(source, dest, board[source], board[dest],
                                   NONE, NONE, special);
    b &= b - 1;
  }
}
...
// for enPassant pawn captures, where one knows the destination in advance and
// generates moves to it.
void CBoard::serializeFromDest(mv **moveList, BB b, ind dest, ind defender,
                               ind special) {
  while (b) {
...
// The end of a moveList array is signified by a 0 move. This helper adds that 0
// move to the list.
void CBoard::closeMoveList(mv **moveList) { *((*moveList)++) = 0; }
