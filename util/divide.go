...
func getDifferingMoveSequence(depth int, p1, p2 process) string {
	...
	fmt.Println("DD: ", depth, difference)
...
func main() {
	...
	process1 := NewProcess(*engine1)
	process2 := NewProcess(*engine2)
	for i := 0; i < 7; i++ {
		<-process1.Stdout
		<-process2.Stdout
	}
	...
	process1.Stdin <- "force"
	process2.Stdin <- "force"

	// set boards if necessary

	fmt.Println(getDifferingMoveSequence(4, process1, process2))
}
